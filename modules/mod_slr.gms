...
GMSLR.l(t) = 0;
*GMSLR.lo(t) = 0;#GMSLR.up(t) = 10;


variable SLR(w,t) 'Sea level rise [W/m2]';
SLR.l(w,t) = 0;
*SLR.lo(w,t) = 0;#SLR.up(w,t) = 10;
...
eqslr_gris(t)..
    SLR('gris',t) =e= sum(tt$(year(tt) < year(t)), 5 * (71.5 * TATM(tt) + 20.4 * TATM(tt)**2 + 2.8 * TATM(tt)**3)) / 3.61e5 + slr_gris0;

eqslr_antis(t)..
    SLR('antis',t) =e= sum(tt$(year(tt) < year(t)), 5 * (0.00074 + 0.00022 * TATM(tt))) + slr_antis0;

eqslr_mg(t)..
    SLR('mg',t) =e= sum(tt$(year(tt) < year(t)), 5 * (0.0008 * TATM(tt)) * (1 - SLR('mg',tt) / 0.41)**1.646) + slr_mg0;

eqtemp_ml(t)..
    TEMP('ml',t) =e= TATM(t) / slr_beta;

eqtemp_tc(t)..
    TEMP('tc',t+1)$(ORD(t) gt 1) =e= TEMP('tc',t) + (slr_w_e / slr_d_tc * (TEMP('ml',t) - TEMP('tc',t))) * nbsecyear * tlen(t) -
                                           (slr_w_d / slr_d_tc * (TEMP('tc',t) - TEMP('dp',t))) * nbsecyear * tlen(t);

eqtemp_dp(t)..
    TEMP('dp',t+1)$(ORD(t) gt 1) =e= TEMP('dp',t) + (slr_w_d / slr_d_dp * (TEMP('tc',t) - TEMP('dp',t))) * nbsecyear * tlen(t);
...
Loop(t,
TEMP('tc',t+1) = TEMP.l('tc',t) + (slr_w_e / slr_d_tc * (TEMP.l('ml',t) - TEMP.l('tc',t))) * nbsecyear * tlen(t) -
                                           (slr_w_d / slr_d_tc * (TEMP.l('tc',t) - TEMP.l('dp',t))) * nbsecyear * tlen(t);
TEMP('dp',t+1) =e= TEMP.l('dp',t) + (slr_w_d / slr_d_dp * (TEMP.l('tc',t) - TEMP.l('dp',t))) * nbsecyear * tlen(t);
);   
SLR.l('thermo',t) =e= slr_gamma_ml * TEMP.l('ml',t) * slr_d_ml +
                          slr_gamma_tc * TEMP.l('tc',t) * slr_d_tc +
                          slr_gamma_dp * TEMP.l('dp',t) * slr_d_dp;
SLR.l('mg',t) =e= sum(tt$(year(tt) < year(t)), 5 * (0.0008 * TATM.l(tt)) * (1 - SLR.l('mg',tt) / 0.41)**1.646) + slr_mg0;
SLR.l('gris',t) =e= sum(tt$(year(tt) < year(t)), 5 * (71.5 * TATM.l(tt) + 20.4 * TATM.l(tt)**2 + 2.8 * TATM.l(tt)**3)) / 3.61e5 + slr_gris0;
SLR.l('antis',t) =e= sum(tt$(year(tt) < year(t)), 5 * (0.00074 + 0.00022 * TATM.l(tt))) + slr_antis0;
